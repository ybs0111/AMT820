...
int CRun_LdPicker::Process_DVC_Place(int nMode, int nWork_Site/*, int nPosition*/, CString strLotNo)//WORK_TOP WORK_MID WORK_BTM
{
	...
	switch(mn_Place_Step)
	{
		...
		case 3000:
			...
			else if(nWork_Site == THD_CARRIER_RBT)// TOP MIDDEL BTM
			{
		...
		case 3420:
			nRet_1 = FAS_IO.Chk_IO_OnOff(st_io.i_Loader_Transfer_Clamp_Off_Check, IO_OFF, st_wait.nOnWaitTime[WAIT_PICKER_CLAMP_ON_OFF], st_wait.nLimitWaitTime[WAIT_PICKER_CLAMP_ON_OFF]); 
			if(nRet_1 == RET_GOOD) //감지가 안되도 안전화 시간까지는 기다리자 
			{
				mn_Place_Step = 4000;
			} 
	...
	switch( mn_Pick_Step )
	{
		...
		case 1000:
			...
			if( nRet_1 == RET_GOOD )
			{
				...
				m_n_FirstTray_X_Num = m_npFindWorkPosYXCPB[1];
			}
		...
		case 3530:
			nRet_1 = FAS_IO.Chk_IO_OnOff(st_io.i_Loader_Transfer_Clamp_Off_Check, IO_ON, IO_STABLE_WAIT, 1000);
			if(nRet_1 == RET_GOOD)
			{
				mn_Pick_Step = 4000;
			}
