...
void CScreen_List_Operation::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RADIO_BARCODE, m_radio_barcode);
...
/////////////////////////////////////////////////////////////////////////////
// CScreen_List_Operation message handlers
...
//	m_group_operation_list.SetBackgroundColor(RGB(201, 201, 151), RGB(255, 255, 204));
	...
	m_group_operation_list.SetCatptionTextColor(BLUE_C);
	m_group_operation_list.SetBorderColor(YELLOW_L);
...
//	m_group_operation_type.SetBackgroundColor(RGB(201, 201, 151), RGB(255, 255, 204));
	...
	m_group_operation_type.SetCatptionTextColor(BLUE_C);
	m_group_operation_type.SetBorderColor(YELLOW_L);
...
{	
	...
	m_edit_operation_content.setFont(-14, FW_BOLD, DEFAULT_PITCH | FF_DONTCARE, "굴림체");
}
...
void CScreen_List_Operation::OnOperation_List_RadioButton_Set()
{
	...
	m_radio_barcode.SetImage(IDB_REDBUTTON, 15);
}
...
void CScreen_List_Operation::OnOperation_List_Full_Path_Set(int n_selected)
{
	...
	case 3 :  // Machine 경로 설정 
		...
		break;
	case 4 :  // Barcode 경로 설정 
		mstr_full_path = mstr_default_path + "Log\\Barcode\\";
		break;
	default :
		break;
	}
...
int CScreen_List_Operation::OnOperation_List_Selected_File_Name_Check()
{
	...
	else 
	{
		...
		{
			m_msg_operation_file.SetWindowText(_T("확장자 입력 오류 발생"));
			return FALSE;
		}
	}
...
void CScreen_List_Operation::OnOperation_List_Display_Selected_Day()
{
	...
	if (m_monthcal_operation.GetSelRange(ctMinRange, ctMaxRange))
	{
		...
		case 3 :  // Machine 출력 파일 이름 정보 설정 
			...
			break;
		case 4 :  // Barcode 출력 파일 이름 정보 설정 
			mstr_date_msg = ctMinRange.Format(_T("%Y%m%d"));
			mstr_chk_file = "BC" + mstr_date_msg;
			break;
		default :
			break;
		}
...
void CScreen_List_Operation::OnRadioBarcode() 
{
	if (mn_type_select != 4)
	{
...
void CScreen_List_Operation::OnBtnOperationFileDel() 
{
	...
	if (OnOperation_List_Selected_File_Name_Check() == FALSE)  return;  // 선택된 파일 이름 정보 정상 여부 검사 함수
	...
	if (mn_response == IDOK)
	{
		mstr_del_file = mstr_full_path + mstr_operation_file_name;  // 삭제할 파일에 대한 [폴더]+[파일명] 정보 설정 
		...
		if (mn_existence != -1)
		{
			...
			OnOperation_List_Create_ImageList();		// 알람 리스트 출력 화면에 사용할 이미지 생성 함수

			// Associate image list to list box
			m_list_operation_file.SetImageList(&mp_operation_image_list);

			OnOperation_List_Init_List(LPCTSTR(mstr_full_path)); // 알람 리스트 출력 화면에 출력할 파일 경로 초기화 작업 함수
			/* ****************************************************************** */

			m_msg_operation_file_name.SetWindowText(_T(""));			// 선택된 파일의 이름을 출력 시킨다.
...
BOOL CScreen_List_Operation::DestroyWindow() 
{
