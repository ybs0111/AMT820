...
void CDialog_Manual_Move::OnBtnEndlessLoop() 
{
	...
	if( st_handler.n_initial == TRUE )
	{
		...
		if( dCurr > ( st_motor[M_HEATSINK_TRANSFER_Y].md_pos[P_HEATSINK_TRANSFER_Y_CARRIER_SAFETY_POS] + COMI.md_allow_value[M_HEATSINK_TRANSFER_Y] ) )
		{
...
int CDialog_Manual_Move::Move_TrayTransfer()
{
	...
	switch( m_move_step[m_nAxisNum] )
	{
		...
		case 3100:
			nRet_1 = COMI.Check_MotPosRange(m_nAxisNum, m_dpTargetPos[m_nAxisNum], COMI.md_allow_value[m_nAxisNum]); 			
			if (nRet_1 != BD_GOOD)
			{
				m_move_step[m_nAxisNum] = 2000;
				break;
		...
		case 1100:
			...
			else
			{
				...
				else
				{
					...
					if( COMI.Get_MotCurrentPos( M_HEATSINK_TRANSFER_Y) > ( st_motor[M_HEATSINK_TRANSFER_Y].md_pos[P_HEATSINK_TRANSFER_Y_CARRIER_SAFETY_POS] + COMI.md_allow_value[M_EPOXY_TRANSFER_Y] ) )
					{
						break;
					}
					mn_RunTopFwdStep = 1200;
...
int CDialog_Manual_Move::Move_UnPressTransfer()
{
	...
	switch( m_move_step[m_nAxisNum] )
	{
		...
		case 100:
			...
			{
				if( mCurrPosX > ( st_motor[m_nAxisNum].md_pos[P_CARRIER_X_INIT_POS] + COMI.md_allow_value[m_nAxisNum] ) )
				{
					m_move_step[m_nAxisNum] = 300;
					break;
				}				
...
// 			}
			...
			if( m_nTrayDeviceGap > 0 && ( mCurrPosX > ( st_motor[m_nAxisNum].md_pos[P_CARRIER_X_UNPRESS_POS] + COMI.md_allow_value[m_nAxisNum] ) ||
				mCurrPosX < ( st_motor[m_nAxisNum].md_pos[P_CARRIER_X_UNPRESS_POS] - COMI.md_allow_value[m_nAxisNum] ) ) )
			{
			...
			{
				if( ( mCurrPosX < ( st_motor[m_nAxisNum].md_pos[P_CARRIER_X_UNPRESS_POS] + COMI.md_allow_value[m_nAxisNum] ) ) &&
					( mCurrPosX > ( st_motor[m_nAxisNum].md_pos[P_CARRIER_X_UNPRESS_POS] - COMI.md_allow_value[m_nAxisNum] ) ) )
				{
					m_move_step[m_nAxisNum] = 2100;
					break;
...
// 			}
		...
		case 2700:
			if( mCurrPosY > ( st_motor[m_nAxisNum].md_pos[P_PRESS_Y_INIT_POS] + COMI.md_allow_value[m_nAxisNum] )  )
			{
				//020008 1 0 "M_PRESS_Y_MOTOR_IS_NON_SAFETY_POS."
...
// 
// 		case 3200:
// 			nRet_1 = COMI.Check_MotPosRange(m_nRobot_X, st_motor[m_nRobot_X].md_pos[P_HEATSINK_TRANSFER_X_INSPECT_GABAGE_POS], COMI.md_allow_value[m_nRobot_X]);
// 			nRet_2 = COMI.Check_MotPosRange(m_nRobot_Y, st_motor[m_nRobot_Y].md_pos[P_HEATSINK_TRANSFER_Y_INSPECT_GABAGE_POS], COMI.md_allow_value[m_nRobot_Y]);
// 			if(nRet_1 != BD_GOOD || nRet_2 != BD_GOOD)
// 			{
...
// 			}
		...
		case 4000:
			if( nHeatSinkRbt_Dvc_Req[THD_DISPENSOR_PRBT][0] == CTL_REQ && 
				nHeatSinkRbt_Dvc_Req[THD_DISPENSOR_PRBT][1] == WORK_PLACE)
			{
				nHeatSinkRbt_Dvc_Req[THD_DISPENSOR_PRBT][0] = CTL_READY;
				nHeatSinkRbt_Dvc_Req[THD_DISPENSOR_PRBT][1] = WORK_PLACE;
...
// 					m_strPartNo[0] = st_picker[THD_LOAD_WORK_RBT].st_pcb_info[i].strPartNo;
		...
		case 5400:
			...
			if( st_carrier_buff_info[TOPSHIFT_BUFF_LOADER_RECEIVE].n_exist[m_LoadBufferPos] == CTL_YES )// && (st_buffer_info[THD_LDULD_CARRIER_BUFF].st_pcb_info[m_n_FirstTray_Y_Num].strLotNo == m_strFindLotNo) )
			{			
				...
				//nPos
				int nPos = 0;
				if( st_carrier_buff_info[TOPSHIFT_BUFF_LOADER_RECEIVE].n_exist[m_n_FirstTray_Y_Num] == CTL_YES)
				{
// 					m_strPartNo[0] = st_buffer_info[THD_LD_ALIGN_BUFF].st_pcb_info[0].strLotNo;
// 					m_strPartNo[0] = st_buffer_info[THD_LD_ALIGN_BUFF].st_pcb_info[0].strPartNo;
				}
