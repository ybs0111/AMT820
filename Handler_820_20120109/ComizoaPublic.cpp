...
//3. 모터 스피드를 셋팅한다 cmmCfgSetSpeedPattern(nAxis, nSpeedMode, d_work, d_accel, d_decel);
	...
	{
		nRet = cmmGnInitFromFile(s_file); //("c:\\AMT820\\Setting\\amt820.cme2");
		if (nRet != cmERR_NONE)
		{
			return BD_ERROR; //MOT_OPEN_FAIL;
		}
...
// ******************************************************************************
int CComizoaPublic::Check_MotPosRange(int n_Axis, double d_Distance, double d_Range) // Range를 Int에서 Double로 바꿈. 2K14/11/04/ViboX
{
	int    nRet = BD_ERROR;
	double dGetActualPos=0;
	double dP = 0, dN = 0;
...
// 	nFlag = cmmCfgSetSpeedPattern_T(n_Axis, n_SpeedMode, d_Speed, md_spd_vel[n_Axis][1], md_spd_vel[n_Axis][2]);
	nRet = cmmCfgSetSpeedPattern(n_Axis, n_SpeedMode, d_Speed, md_spd_vel[n_Axis][1], md_spd_vel[n_Axis][2]);
...
int CComizoaPublic::Set_MotSpeed(int n_SpeedCase, int n_Axis, int n_SpeedMode, double d_work, double d_accel_time, double d_decel_time) 
{
	...
	{			 
		...
		nRet = cmmHomeSetSpeedPattern(n_Axis, cmSMODE_T, d_work, d_accel_time/(double)1000, d_decel_time/(double)1000, 1);
	...
	else //정상동작시 속도셋팅모드 
	{
		...
		if (mn_run_status == CTL_dRUN )//|| mn_run_status == CTL_dMAINT)	// 2015.07.01 - Bredmin.
		{
...
//	int    nSafetyCheck = 0, Ret, i=0;
	...
	for(i=0; i<l_AxisCnt; i++)
	{
		...
		Set_MotSpeed_Ratio(lp_AxisNum[i], cmSMODE_S, dp_SpdRatio[0], dp_SpdRatio[1], dp_SpdRatio[2]);//V_0.1.4 090726 cmSMODE_KEEP 
...
//		if ( Get_MotPower(n_Axis) == BD_GOOD )  
	...
	case 310 :  // MOTION DONE 검사 		
		...
		{//st_motor[n_Axis].mn_homecheck_method == 6)	// -EL로 홈체크 하는 모터일 경우에는 ZeroReturn은 보지 않는다.
...
int CComizoaPublic::Check_Motion_State(int n_Axis, long l_State) 
{
	...
	nRet = cmmStReadMotionState(n_Axis, &lMotionState); 
	...
	{//정상적으로 처리됨 
		...
		else if(0 > lMotionState) //음수 값이면 에러처리이다 
		{
			...
			if(st_motor[n_Axis].n_sd_mv_chk == 1) //sd를 조건을 확인하기 위해 이동한다 
			{
				ml_alarmcode_id = nRet;    
				CTL_Lib.Motor_Error_Occurrence(3, n_Axis, ml_alarmcode_id, _T("Check_Motion_State_cmmStReadMotionState_lMotionStateValue_Error"));
			}
