...
//	}
	...
	if (n_response == IDOK)
	{
		...
		if (st_handler.cwnd_list != NULL)
		{
			...
			printf(st_msg.c_normal_msg, st_other.str_normal_msg);
...
// 			st_msg.mstr_barcode = strTemp.Mid(nTmp+1,2);
	...
	//	m_nBarcodeCount++;
		/*st_sync.n_barcode_read_serial_num[0][0] = atoi(strData);*/
	}
...
void CMainFrame::OnMain_Snd_Serial(WPARAM wParam, LPARAM lParam)
{
	...
	// 송신 데이터 미존재 유무 검사한다                                          
	// **************************************************************************
	if (st_serial.str_snd[wParam -1] == _T(""))  // 송신 데이터 공백 검사
	{
		if (st_handler.cwnd_list != NULL)       // 리스트 바 화면 존재
		{
			//			st_serial.mcomm_err[wParam-1] = _T("[송신 데이터] 전송 데이터 공백 에러 발생");
			st_serial.comm_err[wParam -1] = _T("[Send Data Error] DATA Empty transfer error.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, wParam -1, SEND_MSG);  // 에러 메시지 출력 요청
		}
		
		st_serial.n_snd_chk[wParam -1] = COM_ERROR;  // 송신 에러 플래그 설정
		return;
	}
	// **************************************************************************
	...
	if (n_serial_chk == 0)        // 정상 송신
	{
		...
		st_serial.n_snd_chk[wParam -1] = CTL_YES;
	}
	else if (n_serial_chk == -1)  // 쓰기 에러
	{
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			...
			st_serial.comm_err[wParam] = _T("[TR DATA] Serial Port write error.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, wParam -1, SEND_MSG);  // 에러 메시지 출력 요청
		}
		
		st_serial.n_snd_chk[wParam -1] = -1;  
	}
	else if (n_serial_chk == -2)  // TIME OUT
	{
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			...
			st_serial.comm_err[wParam -1] = _T("[TR DATA] Serial port TIME OUT.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, wParam -1, SEND_MSG);  // 에러 메시지 출력 요청
		}
		
		st_serial.n_snd_chk[wParam -1] = -2;
	}
	// **************************************************************************
}
///
