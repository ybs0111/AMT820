...
*/
	...
	if (mstr_old_msg == st_other.c_normal_msg)//st_other.str_abnormal_msg) //2011.0829 test  if(st_other.str_old_abnormal_msg == st_other.str_abnormal_msg)
	{
	...
	if (st_other.c_normal_msg != _T(""))
	{
		ml_list_msgterm_time_normal[0] = GetCurrentTime();
		
		mstr_old_msg =	st_other.c_normal_msg;
		mstr_msg   =	st_other.c_normal_msg;
	}
	else
	{
...
*/
	...
	//2011.0504 추가 일정 시간 이상일때만 메시지 내용 확인하여 파일로 저장 
	if (mstr_old_msg == st_other.c_abnormal_msg)//st_other.str_abnormal_msg) //2011.0829 test  if(st_other.str_old_abnormal_msg == st_other.str_abnormal_msg)
	{
		//2011.0504 추가 같은 메세지가 나오는 문제 처리 
	...
	if (st_other.c_abnormal_msg != _T(""))
	{
		ml_list_msgterm_time_abnormal[0] = GetCurrentTime();
		
		mstr_old_msg =	st_other.c_abnormal_msg;
		mstr_msg   =	st_other.c_abnormal_msg;
	}
	else
	{
		mstr_msg = "Display Message is Empty";
