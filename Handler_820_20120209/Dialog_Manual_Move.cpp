...
void CDialog_Manual_Move::OnBtnEndlessLoop() 
{
	...
	if( st_handler.n_initial == TRUE )
	{
		...
		st_basic.n_mode_jig = WITHOUT_JIG;
		Init_Timer();
		...
		if(mn_move_carrier == 3)//only load rbt ELV
		{
		...
		if( mn_move_carrier == 5)// Only HeatSink + vision
		{
			int m,x,y;
...
// 			}
			...
			Run_UnPress_Robot.Set_UnPress_PushClamp_OnOff(0, OFF);
...
// 			}
		...
		case 2620:
			if( Run_UnPress_Robot.Chk_UnPress_PushClamp_OnOff(0, OFF) == RET_GOOD && 
				Run_UnPress_Robot.Chk_UnPress_PushClamp_OnOff(1, OFF ) == RET_GOOD )
			{
...
int CDialog_Manual_Move::Move_EpoxyTransfer()
{
	...
	case 1100:
		...
		if( nCarrierRbt_Dvc_Req[THD_EPOXY_RBT][0] == CTL_REQ && nCarrierRbt_Dvc_Req[THD_EPOXY_RBT][1] == WORK_PLACE )
		{
...
int CDialog_Manual_Move::Move_BuffDispensor()
{
	...
	switch( m_move_step[m_nAxisNum] )
	{
		...
		case 1100:
			if( nHeatSinkRbt_Dvc_Req[THD_DISPENSOR_PRBT][0] == CTL_READY && nHeatSinkRbt_Dvc_Req[THD_DISPENSOR_PRBT][1] == WORK_PLACE)
			{
...
// 						st_tray_info[THD_LD_TRAY_PLATE].st_pcb_info[y][x].strPartNo	= st_tray_info[THD_LD_STACKER].strPartNo;
		...
		case 6100:			
			nLdWorkRbt_Dvc_Req[THD_LD_TRAY_PLATE][0] = CTL_REQ; //트레이 교체를 요청 
			nLdWorkRbt_Dvc_Req[THD_LD_TRAY_PLATE][1] = WORK_PICK; //트레이 집어가라 요청 
			m_move_step[m_nAxisNum] = 6200;
			break;

		case 6200:
			if(nLdWorkRbt_Dvc_Req[THD_LD_TRAY_PLATE][0] == CTL_READY) //트랜스퍼가 트레이를 집고 셋팅 
			{
				nRet_1 = COMI.Get_MotIOSensor(M_TRAY1_Z, MOT_SENS_SD); 	
				if(nRet_1 == BD_ERROR) //트레이가 감지 안된 상태 
				{
					nLdWorkRbt_Dvc_Req[THD_LD_TRAY_PLATE][0] = CTL_CLEAR;
					nLdWorkRbt_Dvc_Req[THD_LD_TRAY_PLATE][1] = CTL_CLEAR; 
...
// 			}
		...
		case 3000:
			if( nCarrierRbt_Dvc_Req[THD_LD_ALIGN_BUFF][0] == CTL_READY && 
				nLdWorkRbt_Dvc_Req[THD_LD_ALIGN_BUFF][1] == WORK_PLACE )
			{
				m_move_step[m_nAxisNum] = 3100;
			}
...
// 					m_strPartNo[0] = st_picker[THD_LOAD_WORK_RBT].st_pcb_info[i].strPartNo;
		...
		////////////////////////////////
		case 5000:
			if( nCarrierRbt_Dvc_Req[THD_LOAD_WORK_RBT][0] == CTL_REQ && nLdWorkRbt_Dvc_Req[THD_LOAD_WORK_RBT][1] == WORK_PLACE )
			{
				m_move_step[m_nAxisNum] = 5100;
			}
			break;
